...
class DenseNet(nn.Module):
    ...
    def __init__(
        ...
        self.features = nn.Sequential(OrderedDict([
            ('conv0', nn.Conv2d(ch_in, num_init_features, kernel_size=7, stride=2,
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        features1 = self.features.relu0(features)
        features2 = self.features.pool0(features1)
        features = self.features.denseblock1(features2)
        features3 = self.features.transition1(features)
        features = self.features.denseblock2(features3)
        features4 = self.features.transition2(features)
        features = self.features.denseblock3(features4)
        ...
        features5 = F.relu(features, inplace=True)
...
def debug():
    data = torch.zeros([1, 1, 256, 256])
    param = {'ch_in': 1}
    model = densenet121(**param)
    out = model(data)
